//! VBlank throttling.
//!
//! Ported from niri. Some buggy drivers deliver VBlanks way earlier than necessary.
//! This helper throttles the VBlank in such cases to avoid tearing and to get more
//! consistent timings.
...
impl VBlankThrottle {
    ...
    /// Check if a VBlank should be throttled.
    ///
    /// Returns `true` if the VBlank was throttled (deferred via timer), meaning the
    /// caller should NOT process it now. Returns `false` if the VBlank should be
    /// processed normally.
    pub fn throttle(
        &mut self,
