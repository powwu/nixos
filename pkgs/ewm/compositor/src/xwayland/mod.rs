...
//
// X11 display allocation and socket management for xwayland-satellite.
// Creates lock files, binds abstract + unix sockets, and provides RAII cleanup.

use std::os::fd::OwnedFd;
use std::os::unix::net::{SocketAddr, UnixListener};

use anyhow::{anyhow, ensure, Context as _};
use smithay::reexports::rustix::fs::{mkdir, unlink, OFlags};
...
fn ensure_x11_unix_perms() -> anyhow::Result<()> {
    ...
    let tmp = lstat(TMP_UNIX_DIR).context("error checking /tmp directory permissions")?;
        ...
        x11_tmp.st_uid == tmp.st_uid || x11_tmp.st_uid == rustix::process::getuid().as_raw(),
